...
pub fn render_toolbar(frame: &mut Frame<CrosstermBackend<Stdout>>, rect: Rect, state: &mut State) {
    ...
    match state.selected.get() {
        ...
        StateSelected::Contact => {
            ...
            match state.contacts.current.selected.get() {
                ContactSelected::Fn => text.push_str(single),
    ...
    frame.render_widget(Paragraph::new(Text::raw(text)).block(Block::default().title("Commands").borders(Borders::ALL).style(Style::default().fg(PRIMARY))), rect);
}
