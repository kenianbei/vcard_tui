...
use crate::state::popup::property_tel::{PopupTelSelected, PropertyTelState};
...
pub fn render_property_tel_popup(state: &mut PropertyTelState, frame: &mut Frame<CrosstermBackend<Stdout>>, rect: Rect) {
    ...
    render_widget_with_textarea("Type", &state.values.param_type, state.selected.is(PopupTelSelected::ParamType), state.textarea.as_mut(), frame, row1_cols[0]);
    render_widget_with_textarea("Tel", &state.values.tel, state.selected.is(PopupTelSelected::Tel), state.textarea.as_mut(), frame, row1_cols[1]);

    frame.render_widget(Paragraph::new(Text::raw("Save")).block(get_block(state.selected.is(PopupTelSelected::Save), "")).alignment(Alignment::Center), row2_cols[1]);
}
