...
pub fn render_message_popup(state: &mut MessageState, frame: &mut Frame<CrosstermBackend<Stdout>>, rect: Rect) {
    ...
    let row2 = Layout::default().direction(Direction::Horizontal).constraints([Constraint::Percentage(75), Constraint::Percentage(25)]).split(rows[1]);

    frame.render_widget(Paragraph::new(Text::raw(state.value.as_str())), row1[0]);
    frame.render_widget(Paragraph::new(Text::raw("Ok")).block(get_block(true, "")).alignment(Alignment::Center), row2[1]);
}
